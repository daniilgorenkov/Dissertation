...
class PDS:
    ...
    def get_profile_results(config:str,
        ...
        if force == "vertical":    
            ...
            file2 = PDS.get_result(config, way_type,fault,speed, "greb_26", force="side")
            file3 = PDS.get_result(config, way_type,fault,speed, "greb_28", force="side")
            file4 = PDS.get_result(config, way_type,fault,speed, "greb_30", force="side")
            file5 = PDS.get_result(config, way_type,fault,speed, "newwagonw", force="side")
            file6 = PDS.get_result(config, way_type,fault,speed, "greb_24", force="side")
        
        return file1,file2,file3,file4,file5,file6
    
    def plot_profile_results(config:str,
                             way_type:str,
                             fault:str,
                             speed:int,
                             force:str ="vertical"):
    ...
    def get_speed_results(config:str,
        ...
        for v in speed:
            ...
            if force=="vertical":
                file = PDS.get_result(config,way_type,fault,profile=profile,speed=v,force=force)
                results.append(file)
            ...
            for way in way_cfg:
                ...
                for fault in fault_cfg:   
                    ...
                    for wheel in wheel_cfg:
                        clear_output(True)
                        print(f"{wagon}\n{way}\n{fault}\n{wheel}\n------")
                        gen_dict[wagon][way][fault][wheel] = PDS.get_speed_results(wagon,way,fault,wheel,force=force)
            ...
            for way in way_cfg:
                ...
                for fault in fault_cfg:   
                    ...
                    for wheel in wheel_cfg:
                        clear_output(True)
                        print(f"{wagon}\n{way}\n{fault}\n{wheel}\n------")
                        gen_dict[wagon][way][fault][wheel] = PDS.time_indexes(calculations[wagon][way][fault][wheel])
                ...
                for fault in fault_cfg:
                    for wheel in wheel_cfg:
                        for l,speed in zip(range(lenght),speed_cfg):
                            ...
                            splitted_df = PDS.get_splitted_dataframe(calculations[wagon][way][fault][wheel][l],
