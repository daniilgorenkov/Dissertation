...
class ForcesDataset(FileOperator):
    ...
    def create_dataset(self, target: str) -> dict[pd.DataFrame, pd.Series]:
        ...
        ds["y_test"] = y_test
        ds["cat_cols"] = [
            col for col in X.columns if X[col].dtype == "int32" and col not in config.Preprocessor.TARGETS
        ]
        ds["cont_cols"] = [col for col in X.columns if X[col].dtype == "float32"]

        self.save(ds, f"{target}_dataset")
        return ds
...
class Trainer(FileOperator):
    ...
    def model_fit(self, model: CatBoostClassifier, train_pool: Pool, dev_pool: Pool, params: dict):
        buffer = io.StringIO()
    ...
    def log_model_results(self, ds: dict, model: CatBoostClassifier, model_task: str = "fault_target"):
    ...
    def train_models(self):
        ...
        logger.debug(f"[ {self.MODEL_NAME.upper()} ] " f"DEVICE: {config.Common.DEVICE}")
        ...
        logger.debug(
            f"Fault dataset shape: {fault_dataset['X_train'].shape}, Profile dataset shape: {profile_dataset['X_train'].shape}"
        )

        # create pools for fault target
        fault_train_pool = Pool(
            fault_dataset["X_train"], fault_dataset["y_train"], cat_features=fault_dataset["cat_cols"]
        )
        ...
        profile_train_pool = Pool(
            profile_dataset["X_train"], profile_dataset["y_train"], cat_features=profile_dataset["cat_cols"]
        )
        profile_dev_pool = Pool(
            profile_dataset["X_dev"], profile_dataset["y_dev"], cat_features=profile_dataset["cat_cols"]
        )
        ...
        if args.update_params:
            ...
            pbar = tqdm(total=2, desc="Training best models", unit="model")
            model_fault = self.model_fit(
                self.MODEL, fault_train_pool, fault_dev_pool, config.Trainer.FAULT_BOOST_PARAMS
            )
            ...
            model_profile = self.model_fit(
                self.MODEL, profile_train_pool, profile_dev_pool, config.Trainer.PROFILE_BOOST_PARAMS
            )
            self.save(model_profile, "profile_model")
            # model_fault = self.load("fault_model")
            pbar.update(1)
